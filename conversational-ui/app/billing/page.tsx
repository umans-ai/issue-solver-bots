...
export default async function BillingPage() {
  ...
  const portalUrl = process.env.STRIPE_BILLING_PORTAL_URL || FALLBACK_PORTAL_URL;

  const pledge = pledgeRecord
    ? {
        plan: pledgeRecord.plan as 'code_pro' | 'code_max',
        billingCycle: pledgeRecord.billingCycle as 'monthly' | 'yearly',
        status: pledgeRecord.status,
      }
    : null;
...
function UmansLogoMark({ className }: { className?: string }) {
  return (
    <svg
