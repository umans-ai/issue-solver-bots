...
function sanitizeMessageParts(parts: UIMessagePart<any, any>[]) {
  return parts
    ...
    .map((part) => {
      const { id, providerMetadata, callProviderMetadata, ...rest } =
      ...
      if (part.type === 'text' || part.type === 'reasoning') {
        ...
        sanitized.state =
          (part as any).state === 'streaming' ? 'done' : (part as any).state;
      }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const entitlement = await checkChatEntitlements({
      userId: session.user.id,
      plan,
    });
    if (!entitlement.ok) {
  ...
  const stream = createUIMessageStream({
    ...
    onFinish: async ({
      ...
      try {
        ...
        const assistantId =
          responseMessage.id ?? latestAssistant?.id ?? assistantMessageId;
            ...
            messages: [
              {
                ...
                chatId: id,
                role:
                  responseMessage.role ?? latestAssistant?.role ?? 'assistant',
                parts: partsToStore,
                attachments: [],
                createdAt: new Date(),
              },
